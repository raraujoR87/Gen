...
NOTE: now that units 1-6 have landed, the actual module/symbol names differ
slightly from the placeholders below (e.g. backend.marketdata.microstructure
instead of backend.marketdata.metrics, backend.ml.inference.evaluate_spread
as a free function instead of a BimodalArbitrageNet method, and
backend.execution.decision / backend.execution.broken_leg.BrokenLegMitigator
instead of a single backend.execution.engine module). Left as a follow-up:
the importorskip guards mean this currently skips rather than failing CI.
"""
